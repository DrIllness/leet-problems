...
fun buildTree(preorder: IntArray, inorder: IntArray): TreeNode? {
    ...
    inorder.forEachIndexed { i, v ->
        valueToIndexInorder[v] = i
    }

    var preorderIndex = 0
    fun buildNodeRecurs(left: Int, right: Int): TreeNode? {
        if (left > right) return null

        val nodeValue = preorder[preorderIndex++]
        val node = TreeNode(nodeValue)
        val midIndex = valueToIndexInorder[nodeValue]!!

        node.left = buildNodeRecurs(left, midIndex - 1)
        node.right = buildNodeRecurs(midIndex + 1, right)

        return node
    }

    return buildNodeRecurs(0, preorder.size - 1)
}