...
fun calcEquation(equations: List<List<String>>, values: DoubleArray, queries: List<List<String>>): DoubleArray {
    if (equations.isEmpty() || queries.isEmpty() || values.isEmpty()) emptyArray<DoubleArray>()

    val response = DoubleArray(queries.size)
    // populate weighted graph
    val divisionResults = hashMapOf<String, HashMap<String, Double>>()
    var i = 0
    while (i < equations.size) {
        val first = equations[i][0]
        val second = equations[i][1]
        if (divisionResults[first] == null) {
            divisionResults[first] = hashMapOf()
        }
        if (divisionResults[second] == null) {
            divisionResults[second] = hashMapOf()
        }

        divisionResults[first]!![second] = values[i]
        divisionResults[second]!![first] = 1.0.div(values[i])

        i++
    }

    val visitedNodes = hashSetOf<String>()
    fun findWay(start: String, end: String): Double {
        if (start == end) return 1.0
        visitedNodes.add(start)
        val neighbors = divisionResults[start] ?: return -1.0

        for ((neighbor, weight) in neighbors) {
            if (visitedNodes.contains(neighbor)) continue

            val result = findWay(neighbor, end)
            if (result != -1.0) {
    ...
    while (i < queries.size) {
        ...
        val end = queries[i][1]
        if (divisionResults.containsKey(start).not() || divisionResults.containsKey(end).not()) {
            response[i++] = -1.0
            continue
        }

        response[i++] = findWay(start, end)
        visitedNodes.clear()
    }
