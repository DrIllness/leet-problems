...
fun cloneGraph(node: GraphNode?): GraphNode? {
    ...
    val clones = hashMapOf<Int, GraphNode>()
    ...
    while (queue.isNotEmpty()) {
        ...
        for (n in original.neighbors) {
            ...
            if (clones.containsKey(n.`val`).not()) {
                if (clones[n.`val`] == null){
                    clones[n.`val`] = GraphNode(n.`val`)
                }
                queue.add(n)
            }

            clones[original.`val`]?.neighbors?.add(clones[n.`val`])
        }
    }
