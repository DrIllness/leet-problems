...
fun isSubsequence(substring: String, targetString: String): Boolean {
    if (targetString.isEmpty() && substring.isNotEmpty()) return false

    var i = 0
    var j = 0
    var nextChar = substring[j]
    while (i < targetString.length) {
        if (targetString[i] == nextChar) {
            j++
            if (j < substring.length) nextChar = substring[j]
        }
        i++
    }
